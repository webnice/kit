...
type packData struct {
	...
	spaceMap            map[string]uint16
	indexMap            map[uint16](map[string]uint64)
	primaryKeyMap       map[uint16]([]int)
}
...
func newPackData(defaultSpace interface{}) *packData {
	...
	return &packData{
		...
		spaceMap:            make(map[string]uint16),
		indexMap:            make(map[uint16](map[string]uint64)),
		primaryKeyMap:       make(map[uint16]([]int)),
	}
}
...
func (data *packData) indexNo(space interface{}, index interface{}) (uint64, error) {
	...
	case string:
		...
		spaceData, exists := data.indexMap[uint16(spaceNo)]
		if !exists {
