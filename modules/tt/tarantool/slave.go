...
func NewSlave(scheme string, host string, opts ...Options) (s *Slave, err error) {
	...
	if err = s.parseOptions(options); err != nil {
		return nil, err
	}

	// it is discussable to connect to instance in instance creation
	if err = s.connect(scheme, host, &options); err != nil {
		return nil, err
	}
...
func (s *Slave) connect(scheme string, host string, options *Options) (err error) {
	conn, err := newConn(scheme, host, parseOptions(options))
	if err != nil {
		return
	}
	s.c = conn
	s.cr = bufio.NewReaderSize(s.c.conn, DefaultReaderBufSize)
	// for better error checking while writing to connection
	s.cw = bufio.NewWriter(s.c.conn)
	return
}
