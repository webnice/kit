...
const (
	...
	IterEq            = uint64(0) // key == x ASC order
	IterReq           = uint64(1) // key == x DESC order
	IterAll           = uint64(2) // all tuples
	IterLt            = uint64(3) // key < x
	IterLe            = uint64(4) // key <= x
	IterGe            = uint64(5) // key >= x
	IterGt            = uint64(6) // key > x
	IterBitsAllSet    = uint64(7) // all bits from x are set in key
	IterBitsAnySet    = uint64(8) // at least one x's bit is set
	IterBitsAllNotSet = uint64(9) // all bits are not set
)
