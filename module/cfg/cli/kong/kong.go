...
func failField(parent reflect.Value, field reflect.StructField, format string, args ...any) (err error) {
	var name string
...
func Must(ast any, options ...Option) (k *Kong) {
	var err error
...
func New(grammar any, options ...Option) (*Kong, error) {
	...
	for _, dcmd := range k.dynamicCommands {
		...
		err = buildChild(k, k.Model.Node, CommandNode, reflect.Value{}, reflect.StructField{
			Name: dcmd.name,
			Type: v.Type(),
		}, v, tag, dcmd.name, make(map[string]bool))
		if err != nil {
			return nil, err
		}
	}
...
func (k *Kong) MakeFlagValue(f *SimpleValue) (ret []*Flag) {
	var newFlag = &Flag{
		Short: f.Short,
		Value: &Value{
			Name:         f.Name,
			Help:         f.Help,
			OrigHelp:     f.HelpOrig,
			Target:       f.ReflectValue,
			Tag:          f.Tag,
			Mapper:       k.Registry.ForValue(f.ReflectValue),
			DefaultValue: reflect.ValueOf(f.DefaultValue),
		},
	}
	ret = k.MakeFlag(newFlag)

	return
}

func (k *Kong) MakeFlag(f *Flag) (ret []*Flag) {
	f.Flag, k.helpFlag = f, f
	return []*Flag{f}
}
