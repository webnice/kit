...
func (v *Value) Parse(scan *Scanner, target reflect.Value) (err error) {
	...
	if err = v.Mapper.Decode(&DecodeContext{Value: v, Scan: scan}, target); err != nil {
		err = Errors().DecodeValueError(v.ShortSummary(), err)
		return
	}
	v.Set = true
...
func (v *Value) Reset() error {
	...
	if v.Tag.Env != "" {
		...
		if envar != "" {
			...
			if err != nil {
				return Errors().DecodeValueEnv(err, v.Tag.Env, envar)
			}
			return nil
		}
	}
