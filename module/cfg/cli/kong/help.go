...
func printNodeDetail(w *helpWriter, node *Node, hide bool) {
	...
	if len(node.Positional) > 0 {
		w.Print("")
		w.Print(w.context.labelArguments)
	...
	if len(cmds) > 0 {
		...
		if w.Tree {
			w.Print("")
			w.Print(w.context.labelCommands)
			writeCommandTree(iw, node)
		} else {
			groupedCmds := collectCommandGroups(w, cmds)
...
func collectCommandGroups(w *helpWriter, nodes []*Node) []helpCommandGroup {
	...
	if ungroupedNodes, ok := nodesByGroup[""]; ok {
		out = append(out, helpCommandGroup{
			Metadata: &Group{Title: w.context.labelCommands},
