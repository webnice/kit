...
Command-line arguments are mapped to values via the Mapper interface:

```go
// A Mapper knows how to map command-line input to
...
All builtin types (as well as a bunch of useful stdlib types like `time.Time`) have mappers registered by default. Mappers for custom types can be added using `kong.??Mapper(...)` options. Mappers are applied to fields in four ways:
