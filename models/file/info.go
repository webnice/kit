...
func (fl *impl) GetInfoSha512(fn string) (inf *InfoSha512, err error) {
	...
	inf = new(InfoSha512)
	if fh, err = os.OpenFile(fn, os.O_RDONLY, os.FileMode(defaultFileMode)); err != nil {
		err = fmt.Errorf("open file %q, error: %s", fn, err)
		return
	}
	defer func() { _ = fh.Close() }()
	s512 = sha512.New()
	if inf.Size, err = io.Copy(s512, fh); err != nil {
		err = fmt.Errorf("read file %q, error: %s", fn, err)
		return
	}
	if fi, err = fh.Stat(); err != nil {
		err = fmt.Errorf("get information about file %q, error: %s", fn, err)
		return
	}
	if inf.Size != fi.Size() {
